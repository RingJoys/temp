...
class ClientA(Client):
    def __init__(self, X, config, initial_weights=None):
    ...
    def task_1(self, client_B_name):
        ...
        encrypted_u_a = np.array(multiprocessing.Pool().map(public_key.encrypt, u_a))
        encrypted_z_a_square = np.array(multiprocessing.Pool().map(public_key.encrypt, z_a_square))
...
class ClientB(Client):
    def __init__(self, X, y, config, initial_weights=None):
        super().__init__(config)
        self.X = X
        self.y = y
        if initial_weights is not None:
    ...
    def task_1(self, client_A_name):
        ...
        try:
            ...
            encrypted_u_b = np.array(multiprocessing.Pool().map(public_key.encrypt, u_b))
...
def load_data(subset_size):
    ...
    X = dataset.drop(columns=['Exited'])  # Assuming 'target' is the column to be predicted
    y = dataset['Exited']  # Replace 'target' with the actual column name
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_subset, y_subset, random_state=1)
...
def vertical_logistic_regression(X, y, X_test, y_test, config, weight_a, weight_b):
    """
    ...
    ## 各参与方的初始化
    client_A = ClientA(XA, config, weight_a)
    print("Client_A successfully initialized.")
    client_B = ClientB(XB, y, config, weight_b)
